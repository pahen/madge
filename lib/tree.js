...
class Tree {
				...
				return stat(srcPath)
					.then((stats) => {
						...
						walk.sync(srcPath, (filePath, stat) => {
							...
							if (!files.includes(filePath) && this.config.fileExtensions.includes(ext)) {
								files.push(filePath);
							}
						});
					});
	...
	generateTree(files) {
		...
		for (const npmKey in npmPaths) {
			...
			for (const npmPath of npmPaths[npmKey]) {
	...
	exclude(tree, excludeRegExp) {
		...
		return Object
			.keys(tree)
			.filter((i) => regExpFilter(i))
			.reduce((acc, id) => {
				acc[id] = tree[id].filter((i) => regExpFilter(i));
				return acc;
			}, {});
	}
