 ...
 * @return {undefined}
 */
module.exports.circular = function (circular, modules, opts) {
	if (opts.json) {
	...
	const foundMsg = 'found in ' + fileCount + ' ' + (fileCount > 1 ? 'files' : 'file');

	if (!circular.length) {
		console.log(green('✔ No circular dependency ' + foundMsg));
	} else {
		console.log(red('✘ ' + cyclicCount + ' circular ' + (cyclicCount > 1 ? 'dependencies ' : 'dependency ') + foundMsg));
		circular.forEach((path, idx) => {
			path.forEach((module, idx) => {
				if (idx) {
					process.stdout.write(grey(' -> '));
				}
				process.stdout.write(cyan(module));
			});
