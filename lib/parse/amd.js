 ...
 * @param {Object} parent
 */
const AMD = module.exports = function (src, opts, parent) {
...
AMD.prototype.parseFile = function (filename) {
	try {
		...
		if (/define|require\s*\(/m.test(fileData.src)) {
			amdetective(fileData.src, {findNestedDependencies: this.opts.findNestedDependencies}).map((obj) => {
				...
				deps.filter((id) => {
					...
					const depFilename = path.resolve(path.dirname(fileData.filename), id);
